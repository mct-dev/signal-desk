...
test('builds concrete briefs from niche pain plus spend evidence and downranks generic trend chatter', () => {
  const evidence = [
    ...
    item({
      ...
      url: 'https://stackoverflow.example/soc2-evidence',
      tags: ['soc2', 'automation', 'compliance'],
      metrics: { viewCount: 940, answerCount: 0, score: 6, isAnswered: false },
    }),
    item({
  ...
  const briefs = buildOpportunityBriefs(
    [
      ...
      item({
        ...
        summary: 'No accepted answer and the retry state is difficult to inspect.',
        url: 'https://stackoverflow.example/webhook-retries',
        tags: ['webhooks', 'retries'],
        metrics: { isAnswered: false, viewCount: 410 },
      }),
      item({
        source: 'GitHub Issues',
        sourceType: 'developer-pain',
